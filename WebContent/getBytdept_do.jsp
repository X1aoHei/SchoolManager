...
<header><div id="text"><img src="image/01.jpg" width="50" height="50"/></div><jsp:include page="addone.jsp"></jsp:include></header>
	...
	<div id="aside01">
		  ...
		  <%
		  String tdept = request.getParameter("querytdept");
		  String err="";
		  String nullerr = "";
		  %>
		  <jsp:useBean id="teas" class="Bpo.TeacherBpo" scope="session"/>
		<%List<TeacherBean> teach = teas.getBytdept(tdept); %>
		<table>
			...
			if(tdept==null||tdept.equals("")){
				...
				for(int i=0;i<teach.size();i++){
				 ...
				 %>
					<tr><td><%=tea.getTno() %></td><td><%=tea.getTname() %></td><td><%=tea.getSex() %></td><td><%=tea.getTdept() %></td><td><%=tdept %></td><td><%=tea.getTdirect() %></td></tr>
					<%} 
			}else{
				err = "未查询到该用户，请重新输入";
			}%>
